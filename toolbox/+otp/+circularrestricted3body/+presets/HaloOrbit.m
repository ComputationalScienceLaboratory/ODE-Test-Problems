classdef HaloOrbit < otp.circularrestricted3body.CR3BPProblem
    ...
    methods
        function obj = HaloOrbit(varargin)
            ...
            % Corrected : boolean
            %    True (default) uses a modified version to better work
            %    with the physical constants and matlab time integrators.
            %    False uses the original table from :cite:p:`Spr21`.
            ...
            p.addParameter('Corrected', true, @islogical);
            p.parse(varargin{:});
            results = p.Results;

            if results.Corrected
                T = gethalotablecorrected(results.OrbitType);
            else
...
function T = gethalotablecorrected(orbittype)
...
st = load('haloorbits.mat');

T = st.orbits.(orbittype);
