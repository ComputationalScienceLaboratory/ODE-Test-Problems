classdef NBodyParameters < otp.Parameters
    ...
    properties
        ...
        Masses %MATLAB ONLY: (:,1) {otp.utils.validation.mustBeNumerical} = 1

        %GravitationalConstant is exactly what it says on the box
        GravitationalConstant %MATLAB ONLY: (1,1) {otp.utils.validation.mustBeNumerical} = 1

        %SofteningLength is a heuristic parameter that avoids singularities
        SofteningLength %MATLAB ONLY: (1,1) {otp.utils.validation.mustBeNumerical, mustBePositive} = 0
    end
